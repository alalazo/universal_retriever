...
namespace universal_retriever
{
    ...
    class Frontend
    {
      public:
         ...
         */
        template <class T> T retrieve(const std::string& name, const std::string& key)
        {
            try
            {
...
#if BOOST_VERSION > 105500
                estream << " is not associated with type \"" << boost::typeindex::type_id<T>().pretty_name() << "\""
                        << std::endl;
#endif
         ...
         */
        template <class T> void store(const std::string& name, const std::string& key, const T& value)
        {
            try
            {
            ...
            {
                std::stringstream estream;
                estream << "ERROR : the front-end doesn't know how to manage \"" << name << "\" data types"
                        << std::endl;
